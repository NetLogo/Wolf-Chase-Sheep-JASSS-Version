...
to go
  if done? [stop]
  ask sheep [sheep-move]
  ask wolves [wolf-move]
  tick
end


to sheep-move
  turn-away-from one-of wolves sheep-max-turn-degrees
  forward sheep-speed
end

to wolf-move
  turn-towards one-of sheep wolf-max-turn-degrees
  forward wolf-speed
  if any? sheep-here [
    ask sheep-here [die]
    ]
...
to turn-away-from [an-agent max-turn-degree]
let angle (towards an-agent - heading) mod 360
  if angle > 180 [set angle angle - 360]
  (ifelse
    angle < -15 [rt max-turn-degree]
    angle > 15 [lt max-turn-degree]
  )
end
to turn-towards [an-agent max-turn-degree]
  let angle (towards an-agent - heading) mod 360
  if angle > 180 [set angle angle - 360]
  (ifelse
    angle < -15 [lt max-turn-degree]
...
to-report done?
  ;; two conditions under which the model is done: all sheep are dead, or sheep is on border of world
  report not any? sheep or any? sheep with [abs round xcor = max-pxcor or abs round ycor = max-pycor]
end

to-report wolf-caught-sheep?
  report not any? sheep
end
@#$#@#$#@
GRAPHICS-WINDOW
210
