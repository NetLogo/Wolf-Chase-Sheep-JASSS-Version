extensions [ls]
globals [
  chase-model; for levelspace chase model
]
...
to setup
  clear-all
  ls:reset
  (ls:create-interactive-models 1 "Chase Model.nlogo" [ id -> set chase-model id])
  ask patches [
...
to go
  ; stop the simulation if no wolves or sheep
  if not any? turtles [ stop ]
  ifelse show-levelspace-chase? [ls:show chase-model][ls:hide chase-model]
  ask sheep [
    move
...
to eat-sheep
  if any? sheep-here [
    ...
    ;; set the heading of the wolf and the sheep in the chase model, then set up the model
    ls:ask chase-model [
    ...
    ;; if the wolf caught it, add energy and kill the sheep. Else do nothing.
...
This is a model that uses the LevelSpace to explore how the original Wolf Sheep Predation NetLogo Model (Wilensky, 1997) can be expanded on by "zooming in" on the chase-process.

The model uses LevelSpace to open a smaller model called Chase Model.nlogo. Every time a wolf and a sheep meet, they are "sent" to the chase model in order to determine whether the wolf catches the sheep.

Unlike in the original Wolf Sheep Predation model, it is possible to explore fine-grained parameters in the Chase model, because it allows us to adapt the scale of the simulated processes. In the Chase model, for instance, it is possisble to explore how changes to speed, or the agility of a wolf or a sheep, affect survival outcomes.
...
* Hjorth, A., Head, B., Brady, C. & Wilensky, U. (2019). Wolf Chase Sheep Model.
